...
8. Открой создание заявки. Обычно путь такой:
...
9. Выбери простую тему без фото, например:

```text
Інше / Допомога / Доставка
```

10. Заполни текст заявки на украинском:

```text
Готовий допомогти сусідам з доставкою невеликих покупок по Чайці сьогодні після 18:00.
```

11. Если форма просит телефон, введи телефон бота:

```text
+380671000001
```

12. Если форма просит имя, введи:
...
15. После отправки проверь один из вариантов:
...
5. Нажать "+" → выбрать "Допомога"
6. Ввести текст: "Готовий допомогти з доставкою по Чайці"
7. Нажать "Опублікувати"
8. Убедиться что заявка появилась в ленте

ОЖИДАЕМЫЙ РЕЗУЛЬТАТ: заявка создана, статус pending
