    ...
    <div style={listStyles.container}>
      ...
      <div style={listStyles.filters}>
        ...
        >
          ...
          {categories.map((c) => (
            <option key={c} value={c}>{getAdCategoryLabel(c)}</option>
          ))}
        </select>
      </div>
      ...
      <div style={listStyles.list}>
        ...
        {pageTickets.map((t) => {
          const unread = hasAdUnreadFromUser(t);
          ...
          return (
            ...
            >
              ...
              <div style={listStyles.itemMeta}>
                ...
                <span style={listStyles.category}>{getAdCategoryLabel(t.category)}</span>
              </div>
            </div>
          );
        })}
    ...
    <div style={chatStyles.container}>
      ...
      <div style={chatStyles.info}>
        <div style={chatStyles.infoRow}>
          <strong>Категорія:</strong> {getAdCategoryLabel(ticket.category)}
        </div>
        <div style={chatStyles.infoRow}>
...
export function AdChatPage() {
  ...
  const [stats, setStats] = useState<AdStats>({ total: 0, open: 0, today: 0 });
  ...
  useEffect(() => {
    const unsub = subscribeToAdTickets((list) => {
      setTickets(list);
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToAdMessages(selectedTicket.ticketId, setMessages);
    return unsub;
  }, [selectedTicket?.ticketId]);

  // Mark as read when selecting ticket
  useEffect(() => {
    if (selectedTicket && hasAdUnreadFromUser(selectedTicket)) {
      markAdReadByAdmin(selectedTicket.ticketId);
    }
  }, [selectedTicket?.ticketId, selectedTicket?.lastUserMessage]);

  const handleSelect = (t: AdTicket) => {
    setSelectedTicket(t);
  };
  ...
  const handleSendReply = async (text: string) => {
    ...
    setSending(true);
    try {
      await sendAdAdminReply(selectedTicket.ticketId, text);
    } catch (e) {
  ...
  const handleCloseTicket = async () => {
    ...
    try {
      await closeAdTicket(selectedTicket.ticketId);
    } catch (e) {
  ...
  return (
    <div style={pageStyles.container}>
      <h2 style={pageStyles.title}>📢 Рекламний чат</h2>
